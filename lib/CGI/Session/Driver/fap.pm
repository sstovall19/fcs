package CGI::Session::Driver::fap;
...
sub store {
    ...
    if(my $row = $self->{ResultSet}->search({ $self->{IdColName} => $sid }, { limit => 1 })->first)
    {
	$row->update({
		$self->{IdColName}          => $sid,
		$self->{DataColName}        => $datastr,
	});
    }
    else
    {
        $row = $self->{ResultSet}->update_or_create({
		$self->{IdColName}          => $sid,
		$self->{DataColName}        => $datastr,
        });
    }
...
sub retrieve {
	...
	print STDERR $self->{IdColName}, "\n\n";
    my $row = $self->{ResultSet}->search({ $self->{IdColName} => $sid }, { limit => 1 }) || '';

    if($row && $row->first && $row->first->{_column_data}->{$self->{DataColName}})
    {
       return $row->first->get_column($self->{DataColName});
    }
...
sub remove {
    ...
    # or undef on failure.
    return $self->{ResultSet}->search({$self->{IdColName} => $sid})->delete;
}
...
sub traverse {
    ...
    # argument
    my $rs = $self->{ResultSet}->search->get_column($self->{IdColName});
