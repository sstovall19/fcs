/* 
	...
	*/ 
	...
	function validateCategoryForm(){
		$("#form_Terms").validate({
			errorDiv: 'validation-block-Terms',
			ignore: '',
			rules: {
				prePayAmount :{
					number: true,
			...
			messages: {
				prePayAmount :{
					number: "Please enter numerical value only for pay amount",
				},
				prePayPercentage :{
					digits: "Please enter integer value only for pay percentage",
					max: "Maximum value allowed is 100 for pay percentage"
				}
			}
		});
 	...
 	$.fn.display_review = function(settings)
 		{
 						...
 						for(i in data)
 						{
 						  if(typeof i != "undefined" && data.hasOwnProperty(i)){	
 							...
 							if(buy.qty > 0)
 							{
 								...
 								if(isNaN(buy.price))buy.price = 0;
 								t += '<div class="review_amount">$'+parseFloat(buy.price)+'</div>';//Math.ceil(buy.price)
 							}
 							...
 							{
 								if(isNaN(mrc.price))mrc.price = 0; 
 								t += '<div class="review_amount">$'+parseFloat(mrc.price)+'</div>';//Math.ceil(mrc.price)
 							}
 							t += '</td></tr>';
 						  }	
 						}
 			...
 			if(total_price_discount > 0 || total_mrc_discount > 0)
 			{
 				...
 				if(total_price_discount > 0){
 					discounts += '<div class="review_amount">($'+parseFloat(total_price_discount)+')</div></td><td class="last">';//Math.ceil(total_price_discount)
 			    }else{
 			    	discounts += '&nbsp;</td><td class="last">';
 			    }
 				if(total_mrc_discount > 0){
 					 discounts +='<div class="review_amount">($'+parseFloat(total_mrc_discount)+')</div></td>';//Math.ceil(total_mrc_discount)
 				}else{
 					 discounts += '&nbsp;</td>';
 				}
 				 discounts += '</tr>';
 			...
 			totals += '<td><div class="review_amount total">$'+parseFloat(total_price)+'</div></td>';//Math.ceil(total_price)
 			totals += '<td class="last"><div class="review_amount total">$'+parseFloat(total_mrc)+'</div></td>';//Math.ceil(total_mrc)
 			totals += '</tr>';
 			...
 			$(e).append('<div><div class="review_label" category_id="Review" description="Total Cost Of Ownership" style="width: 210px;">Total Cost Of Ownership</div><div class="review_amount total">$' + parseFloat(total_price + total_mrc)+ '</div>');//(Math.ceil(total_price) + Math.ceil(total_mrc) )
