...
jQuery.validator.addMethod("macaddress", function(value, element, params) {
	return this.optional(element) || /^[a-fA-F0-9]{12}$/.test(value);
}, jQuery.validator.format("Invalid MAC address {1}"));
jQuery.validator.addMethod("phone", function(phone_number, element) {
    phone_number = phone_number.replace(/\s+/g, ""); 
	return this.optional(element) ||
		phone_number.match(/^(?:\((\+?\d+)?\)|\+?\d+) ?\d*(-?\d{2,3} ?){0,4}$/);
